...
CHANNEL_USERNAME = "@tyaf90"  # قناة البوت الأساسيّة
SETTINGS_FILE = "settings.json"

bot = telebot.TeleBot(BOT_TOKEN)

# 🗂️ تحميل إعدادات الاشتراك الإجباري
...
# 📩 رسالة الاشتراك الإجباري بقناة البوت
...
# ⚙️ أمر تعيين قناة الاشتراك الإجباري للمجموعة
...
if __name__ == "__main__":
    ...
    # إعداد Webhook
    WEBHOOK_BASE = os.environ.get("RENDER_EXTERNAL_URL", "").rstrip("/")
    if not BOT_TOKEN or not WEBHOOK_BASE:
        print("❌ خطأ: BOT_TOKEN أو RENDER_EXTERNAL_URL غير مضبوطين في المتغيرات.")
    else:
        full_webhook_url = f"{WEBHOOK_BASE}/{BOT_TOKEN}"
        print("🔗 Webhook URL:", full_webhook_url)
        bot.remove_webhook()
